...
def _load_firebird():
    """Lädt Firebird-Client-Lib.

    Zwei Modi:
    1. Embedded (macOS / custom): HS3_FIREBIRD_HOME/lib/ existiert →
       alle .so/.dylib aus diesem Verzeichnis laden, firebird-driver darauf zeigen.
    2. System (Linux, apt install libfbclient2 firebird3.0-utils):
       lib_dir existiert nicht → firebird-driver findet die System-Lib
       automatisch über den Standard-Suchpfad des Betriebssystems.
       Wir setzen driver_config nur explizit wenn ctypes.util.find_library
       einen Pfad liefert.
    """
    global _fb_loaded
    if _fb_loaded:
    ...
    if lib_dir.exists():
        # ── Modus 1: Embedded / Custom-Installation ─────────────────────────
        os.environ["FIREBIRD"] = HS3_FIREBIRD_HOME

        # Abhängigkeiten zuerst laden, dann Client-Lib
        for pattern in [
        ...
        # ── Modus 2: System-Firebird (Linux apt) ────────────────────────────
        # firebird-driver sucht libfbclient automatisch über ld.so.
        # Expliziter Hint verbessert die Zuverlässigkeit, ist aber optional.
        import ctypes.util
        lib_name = ctypes.util.find_library("fbclient")
        if lib_name:
            from firebird.driver import driver_config
            driver_config.fb_client_library.value = lib_name
...
def _restore_bak(bak_path: Path, fdb_path: Path):
    ...
    result = subprocess.run(  # nosec B603 B607  # nosemgrep
        [str(gbak), "-c", "-user", "sysdba", "-password", "masterkey",  # nosemgrep
